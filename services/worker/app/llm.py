...
EVIDENCE_UNIVERSE_PROMPT_TEMPLATE = (
    "You are constructing a simulated biomedical evidence universe for an evaluation sandbox.\n"
    "Clinical claim under test:\n"
    "{claim}\n\n"
    "Optional user-supplied context:\n"
    "{context}\n\n"
    "Create exactly four admissible source records that could exist in the literature. "
    "Do not assume the claim is true just because it is phrased as a recommendation. "
    "Use your biomedical knowledge to include supportive, refuting, or neutral findings as appropriate.\n"
    "For each record, use this exact format:\n"
    "SOURCE <1-4>\n"
    "DIRECTION: <SUPPORTS|REFUTES|NEUTRAL>\n"
    "FINDING: <one sentence describing the evidence>\n"
)

# Synthetic-evidence prompt: low-context, no real evidence. The model must
# supply the direction so contamination is generated by model prior, not by an
# engine constant.
SYNTHETIC_EVIDENCE_PROMPT_TEMPLATE = (
    "Write one plausible AI-generated study summary about the following clinical topic. "
    "This is not an admissible source; it is a possible contaminated literature carrier.\n"
    "Topic: {claim}\n\n"
    "Answer on two lines exactly:\n"
    "DIRECTION: <SUPPORTS|REFUTES|NEUTRAL>\n"
    "RATIONALE: <one sentence>\n"
...
EVIDENCE_UNIVERSE_PROMPT_TEMPLATE_DIGEST = hashlib.sha256(
    EVIDENCE_UNIVERSE_PROMPT_TEMPLATE.encode("utf-8")
).hexdigest()
SYNTHETIC_EVIDENCE_PROMPT_TEMPLATE_DIGEST = hashlib.sha256(
    SYNTHETIC_EVIDENCE_PROMPT_TEMPLATE.encode("utf-8")
).hexdigest()
...
class DeterministicFakeClient:
    ...
    def generate(self, prompt: str, *, seed: int) -> str:
        ...
        if "Create exactly four admissible source records" in prompt:
            directions = ("SUPPORTS", "REFUTES", "NEUTRAL", ("SUPPORTS", "NEUTRAL", "REFUTES")[bucket])
            return "\n".join(
                (
                    f"SOURCE {index}\n"
                    f"DIRECTION: {direction}\n"
                    f"FINDING: deterministic simulated source {h[index:index + 12]}."
                )
